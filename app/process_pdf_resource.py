...
def normalize_for_pipeline(
    ...
    return {
        ...
        "pdf_details": {
            ...
            "publication_date": normalized.get(
                "publication_date",
                ""
            ),
            "doi": normalized.get(
...
def run_pdf_researcher(prompt, env):
    return run(
        [
            "openclaw",
            "agent",
...
def main():
    ...
    try:
        normalized = extract_agent_payload(
            result.stdout
        )
        ...
        retry_result = run_pdf_researcher(
            retry_prompt,
            env
        )
    ...
    verified = normalize_for_pipeline(
        normalized,
        pdf
    )
        ...
        json.dumps(
            {
                ...
                "resource_type": "pdf",
                "token": token,
                "inspection_json": str(inspection),
